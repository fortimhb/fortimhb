...
%%%%% logaritmic model in the simple regression framework
l_y = log(houses(:,2));
l_X = log(houses(:,3));
model_log = fitlm(l_X,l_y)

% functional form
model_log2 = fitlm(l_X,l_y,'quadratic')
% heteroskedasticity
...
%%%%%% heteroskedasticity analysis (illustration in the linear model)
plotResiduals(modelM,'fitted')
% or
res=modelM.Residuals.raw;
fit=modelM.Fitted;
scatter(fit,res)
% heteroskedasticity test
houses_tab.logres2=log(res.^2);
model_var = fitlm(houses_tab,'ResponseVar','logres2','PredictorVars',{'House_Size','Lot_size','Beds','Baths','Stories','Garage'},...
'CategoricalVar',{'Stories'})
% in this case we accept homoskedasticity.

% normality
plotResiduals(modelM)
histogram(modelM.Residuals.Raw)
histfit(modelM.Residuals.Raw)
plotResiduals(modelM,'probability')
% normality test
skewness(modelM.Residuals.Raw)
kurtosis(modelM.Residuals.Raw)
[h,p,jbstat,critval]=jbtest(modelM.Residuals.Raw)

%%%%%% outliers automatic detection and exclusion (warning!!! no outliers here, 
% but in general detection is ok, but I would not use automatic exclusion) 
outl = find(abs(model.Residuals.Raw) > 3*model.RMSE)
model_noout = fitlm(houses_tab,'ResponseVar','Price','PredictorVars',{'House_Size','Lot_size','Beds','Baths','Stories','Garage'},...
'CategoricalVar',{'Stories'},'Exclude',outl)
...
% or
res=model2_logstep.Residuals.raw;
fit=model2_logstep.Fitted;
scatter(fit,res)
% heteroskedasticity test
houses_tab_log.logres2=log(res.^2);
model_var = fitlm(houses_tab_log,'quadratic','ResponseVar','logres2','PredictorVars',{'lnHouse_Size','lnLot_size','Beds','Baths','Stories','Garage'},...
'CategoricalVar',{'Stories'})
